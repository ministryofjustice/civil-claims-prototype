...
module ApplicationHelper
  def button_to_remove_fields(name, form, association)
    form.hidden_field(:_destroy) + link_to_function(name, "remove_fields(this,'#{association}')")
  end
  
  def button_to_add_fields(directory, name, form, association, default_values = nil)
    new_object = form.object.class.reflect_on_association(association).klass.new
    fields = form.fields_for(association, new_object, :child_index => "new_#{association}") do |builder|
      render "#{directory}/#{association.to_s.singularize}_fields", :f => builder, :default_values => default_values
    end
