class DefencesController < ApplicationController
  ...
  # before_action: :get_current_claim, :get_current_defense
  ...
  def personal_details
    ...
    if get_current_defense.nil?
      ...
      @defence = @claim.defenses.create(owner: @user)
      @defence.fill_basic_monthly_expenses!
    end
  ...
  private
    ...
    def get_current_defense
      @defence = @defence || Defense.find_by(claim_id: params[:claim_id], owner_id: session[:user])
    end
